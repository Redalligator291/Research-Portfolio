...
from updated_agile import lemmatize
...
def main():
    ...
    with open(config.destination_test_path, encoding='utf8') as f:
        for line in f:
            # Print metadata
            if line.startswith('# source'):
                ...
                for sent in words_doc.sentences:
                    for i, word in enumerate(sent.words):
                        ...
                        predicted = ud.normalize('NFC', predicted)
                        gold = ud.normalize('NFC', gold)

                        # Fallback strategy for no prediction
                        if predicted.strip() == "":
                            predicted = text
        ...
        print("{} out of {} correct ({}%)".format(corr_total, corr_total + incorr_total,
                                                  round(corr_total / (corr_total + incorr_total) * 100, 2)))
