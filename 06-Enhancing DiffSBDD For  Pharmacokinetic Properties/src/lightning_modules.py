...
class LigandPocketDDPM(pl.LightningModule):
    def __init__(
        ...
        self.lipinski_weight = lipinski_weight
...
def calculate_lipinski_violations(molecule):
...
def forward(self, data):
    ...
    # Calculate Lipinski's Rule violations
    # Assuming x_lig_hat is a list of RDKit molecule objects
    lipinski_violations = sum(calculate_lipinski_violations(mol) for mol in x_lig_hat)
    lipinski_penalty = self.lipinski_weight * lipinski_violations

    # Add penalty to the loss
    nll = nll + lipinski_penalty

    info['lipinski_penalty'] = lipinski_penalty.mean(0)
